...
int cli_parser(int argc, char **argv, std::tuple<std::string, std::string, std::string> &affordance_args);
...
main(int argc, char **argv)
{
	...
	/************************ CLI HANDLING ************************/
	std::tuple<std::string, std::string, std::string> affordance_args;
	...
	std::string feature_name = std::get<2>(affordance_args);	
	...
	std::cout << "Feature Weights for " << affordance << std::endl;
        printEigenVec(feature_weights_vec);
	...
	std::tuple<int,int,int> feature_tuple = features_map[feature_name];

	int feature_idx = std::get<0>(feature_tuple);
	int start_pos = std::get<1>(feature_tuple) - offset;
	int end_pos = std::get<2>(feature_tuple) - offset;
	...
	<< start_pos << "-" << end_pos << " with length " << range_len << std::endl;
        ...
        std::cout << std::endl << "Feature Weights for the feature:\n" << std::endl;
...
int cli_parser(int argc, char **argv, std::tuple<std::string, std::string, std::string> &affordance_args)
{
	...
	if(vm.count("f"))
	{
	        ...
	        std::map<std::string,std::tuple<int, int, int> >::const_iterator gotFeature = features_map.find (feature);
	        if ( gotFeature == features_map.end() )
	        {
	...
	else
	{
	        ...
	        for ( auto it = features_map.begin(); it != features_map.end(); ++it )
	                std::cout << " " << it->first << ": (" << std::get<1>(it->second) << "," << std::get<1>(it->second) << ")" << std::endl;
	...
	std::get<2>(affordance_args) = feature;
